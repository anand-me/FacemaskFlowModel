function L = compute_mesh_laplacian(vertex,face,laptype,options)

// compute_mesh_laplacian - compute a laplacian matrix
//
//   L = compute_mesh_laplacian(vertex,face,laptype,options);
//
...
//
//   laptype can 'combinatorial', 'distance', 'conformal'.
//
...
if myisfield(options, 'symmetrize')
...
elseif symmetrize==1 & normalize==1
    L = speye(n,n) - diag(sum(W,2).^(-1/2)) * W * diag(sum(W,2).^(-1/2));
elseif symmetrize==0 & normalize==1
    L = speye(n,n) - diag(sum(W,2).^(-1)) * W;
else
