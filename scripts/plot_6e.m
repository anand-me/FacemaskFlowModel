...
[x2,ix]=sort(x1);
x1=x2;
y9=(data_for_figure_8(:,10))';y10=(data_for_figure_8(:,11))';
%%
figure(1)
plot(x1,y9,'^b','LineWidth',1); hold on;
plot(x1,y10,'ok','LineWidth',1); hold on;
%% ~~~~~~~~ Define Legend ~~~~~~~~~~~~~~~~~~~~~~~~~
% legend('$Q_{m,cav}$', '$Q_{m,c}$', 'Interpreter','latex','Location','southwest');
  legend('' ,'' );
    leg = legend('show');
     title(leg,'$c_k = 500 $ $(kg/m^2 \times s)$', 'Interpreter','latex');
grid on
...
xlim([-1.2 1.2])
 ...
 Posleft=Pos(1);
%% ~~~~~~~~ Figure annotation ~~~~~~~~~~~~~~~~~~~~~~~~~
annotation('textbox',[Posleft-0.00 Postop-0.00 0 0],'String','$(e)$', ...
    'EdgeColor','none','FontSize',font_title,'FontName',font_name,'Interpreter','latex' ,...
    'HorizontalAlignment','left','VerticalAlignment','middle'  )
figure(1)
h1 = plot(x1,y9,'^b','LineWidth',1); hold on;
h2 = plot(x1,y10,'ok','LineWidth',1); hold on;
leg = legend('$Q_{m,cav}$', '$Q_{m,c}$', 'Interpreter', 'latex', 'Location', 'southwest');
title(leg, '$c_k = 500 $ $(kg/m^2 \times s)$', 'Interpreter', 'latex', 'FontSize', font_axestitle);
...
xticks([-1.2:0.6:1.2]);
xlim([-1.2 1.2]);
saveas(gcf, 'figure_6e.fig');  
saveas(gcf, 'figure_6e.eps', 'epsc'); 
saveas(gcf, 'figure_6e.jpg', 'jpg'); 
saveas(gcf, 'figure_6e.png', 'png');  
saveas(gcf, 'figure_6e.emf', 'emf'); 
